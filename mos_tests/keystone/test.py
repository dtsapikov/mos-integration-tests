...
class TestKeystone(TestBase):
	...
	self.os_conn.keystone.roles.add_user_role(user.id, role_id, tenant_id)
        ...
        with controller.ssh() as remote:
            remote.check_call(
                "cat openrc >> openrc2 && sed -i 's/export OS_USERNAME='admin'/export OS_USERNAME='testuser'/g' openrc2 && sed -i 's/export OS_PASSWORD='admin'/export OS_PASSWORD='testuser'/g' openrc2")

            result = remote.check_call(
                "source openrc2 && keystone token-get | awk '/ id/{print $4}'")
            token_id = result.stdout_string

            remote.check_call(
                'curl -H "X-Auth-Token: {}" {}/tenants'.format(token_id, endpoint))
        ...
        with controller.ssh() as remote:
            try:
                remote.check_call(
                    'curl -H "X-Auth-Token: {}" http://10.109.1.3:5000/v2.0/tenants'.format(token_id))
